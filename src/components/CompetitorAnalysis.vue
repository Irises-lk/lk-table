...
function onFileChange(e) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      // 形成指定输出段落（中文，数字为整数）
      resultText.value = `本月新增${xx}项竞争对手承揽统计与分析内容。${year}年1-${ii}月，掘进机产品，我司承揽${kk}台，金额${bb}万元；竞争对手承揽${ll}台，金额${cc}万元。特种装备产品，我司承揽${nn}台，金额${ee}万元；竞争对手承揽${oo}台，金额${ff}万元。具体详情见附件：《竞争对手承揽统计与分析表》。`;
      ...
      resultText.value =
        "解析失败：" + (err && err.message ? err.message : err);
    }
  };
...
function findSheet(workbook, keyword) {
  ...
  for (const n of names) {
    if (String(n).replace(/\s+/g, "").indexOf(keyword.replace(/\s+/g, "")) >= 0)
...
function analyzeSheet(ws, prevMonthYear) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    // 安全访问单元格，避免短行或索引越界
    const rawAmount =
      idxAmount >= 0 && row.length > idxAmount ? row[idxAmount] : "";
    ...
    const amountWan = parseAmountWan(
      rawAmount,
      idxAmount >= 0 ? headerRow[idxAmount] : "",
    );
    ...
    if (rawTime != null && rawTime !== "") {
      ...
      if (
        dt &&
        dt.getFullYear() === prevMonthYear.year &&
        dt.getMonth() + 1 === prevMonthYear.month
      ) {
        countPrevMonth++;
      }
    }
...
function findHeaderIndex(headers, type) {
  ...
  for (let i = 0; i < norms.length; i++) {
    ...
    if (type === "amount") {
      ...
      if (/(金额|合同|承揽)/.test(h) && /(万元|元|金额|合同|额)/.test(h))
        return i;
